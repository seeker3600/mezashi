...
_COASTAL_RAFT_SEAWARD_BULGE_TOL_PX = 1.75
_COASTAL_RAFT_SEAWARD_BULGE_RATIO = 0.03
...
@dataclass(frozen=True)
class _CoastalRaftCandidate:
    run: _BerthRun
    berth_stern: bool
    capacity: int
    straightness: float
    seaward_bulge_px: float
    landward_recess_px: float


def _fork_rng(rng: random.Random) -> random.Random:
    forked = type(rng).__new__(type(rng))
    random.Random.__init__(forked)
    if hasattr(rng, "__dict__"):
        forked.__dict__.update(rng.__dict__)
...
def _max_berthed_capacity_for_runs(
    berth_runs: list[_BerthRun],
    ship_gap: float,
    resolution_m: float,
    length_range: tuple[float, float] | None,
    berth_stern: bool,
) -> int:
    """Return the maximum physically feasible ship count across shoreline runs."""
    if not berth_runs:
        return 0
    ...
    return max(
        ...
        for run in berth_runs
    )


def _ordered_berth_mode_attempts(
    berth_runs: list[_BerthRun],
    resolution_m: float,
    length_range: tuple[float, float] | None,
    rng: random.Random,
    *,
    ...
    ship_gap: float,
    min_required_ships: int = 1,
) -> tuple[bool, ...]:
    """Return feasible berth orientations in the order they should be attempted."""
    required = max(1, min_required_ships)
    alongside_capacity = _max_berthed_capacity_for_runs(
        berth_runs,
        ship_gap,
        resolution_m,
        length_range,
    ...
    stern_capacity = _max_berthed_capacity_for_runs(
        berth_runs,
        ship_gap,
        resolution_m,
        length_range,
    ...
    feasible_modes: list[bool] = []
    if alongside_capacity >= required:
        feasible_modes.append(False)
    if stern_capacity >= required:
        feasible_modes.append(True)
    if not feasible_modes:
        return ()
    if len(feasible_modes) == 1:
        return tuple(feasible_modes)

    prefer_stern = rng.random() < max(0.0, min(1.0, berth_stern_prob))
    primary = True if prefer_stern else False
    secondary = not primary
    return tuple(mode for mode in (primary, secondary) if mode in feasible_modes)


def _coastal_raft_run_capacity(
    run: _BerthRun,
    resolution_m: float,
    length_range: tuple[float, float] | None,
    *,
    berth_stern: bool,
    required_cluster_ships: int,
) -> int:
    """Return shoreline feasibility for coastal raft candidates.

    Stern-to clusters consume shoreline span per ship, while alongside clusters
    only require one berthed lead ship and raft the rest offshore.
    """
    linear_capacity = _max_berthed_ships_for_run(
        run.length,
        0.0,
        resolution_m,
        length_range,
    ...
    if linear_capacity <= 0:
        return 0
    return max(1, required_cluster_ships)


def _berth_run_points(run: _BerthRun) -> tuple[tuple[float, float], ...]:
    if not run.segments:
        return ()
    return (run.segments[0].segment[0], *(segment.segment[1] for segment in run.segments))


def _berth_run_shape_metrics(
    run: _BerthRun,
) -> tuple[float, float, float]:
    """Return run straightness plus seaward and landward deviation from the chord."""
    if not run.segments or run.length <= 1e-6:
        return 0.0, 0.0, 0.0

    points = _berth_run_points(run)
    if len(points) < 2:
        return 0.0, 0.0, 0.0
    ...
    straightness = min(1.0, chord_len / run.length)
    chord_tx = chord_dx / chord_len
    chord_ty = chord_dy / chord_len
    normal_candidates = ((-chord_ty, chord_tx), (chord_ty, -chord_tx))
    ref_water_nx = run.segments[0].frame[4]
    ref_water_ny = run.segments[0].frame[5]
    chord_water_nx, chord_water_ny = max(
        normal_candidates,
        key=lambda normal: (normal[0] * ref_water_nx) + (normal[1] * ref_water_ny),
    )

    deviations = [
        ((point_x - start_x) * chord_water_nx) + ((point_y - start_y) * chord_water_ny)
        for point_x, point_y in points[1:-1]
    ]
    seaward_bulge_px = max(0.0, max(deviations, default=0.0))
    landward_recess_px = max(0.0, -min(deviations, default=0.0))
    return straightness, seaward_bulge_px, landward_recess_px


def _ordered_coastal_raft_candidates(
    berth_runs: list[_BerthRun],
    resolution_m: float,
    length_range: tuple[float, float] | None,
    rng: random.Random,
    *,
    berth_stern_prob: float,
    min_required_ships: int = 2,
) -> tuple[_CoastalRaftCandidate, ...]:
    """Return shoreline tight-cluster candidates ordered by run geometry suitability."""
    required = max(2, min_required_ships)
    prefer_stern = rng.random() < max(0.0, min(1.0, berth_stern_prob))
    candidates: list[_CoastalRaftCandidate] = []

    for run in berth_runs:
        straightness, seaward_bulge_px, landward_recess_px = _berth_run_shape_metrics(run)
        for berth_stern in (False, True):
            capacity = _coastal_raft_run_capacity(
                run,
                resolution_m,
                length_range,
                berth_stern=berth_stern,
                required_cluster_ships=required,
            )
            if capacity < required:
                continue
            candidates.append(
                _CoastalRaftCandidate(
                    run=run,
                    berth_stern=berth_stern,
                    capacity=capacity,
                    straightness=straightness,
                    seaward_bulge_px=seaward_bulge_px,
                    landward_recess_px=landward_recess_px,
                )
            )
    ...
    def _seaward_bulge_tolerance(candidate: _CoastalRaftCandidate) -> float:
        return max(
            _COASTAL_RAFT_SEAWARD_BULGE_TOL_PX,
            candidate.run.length * _COASTAL_RAFT_SEAWARD_BULGE_RATIO,
        )

    def _sort_key(candidate: _CoastalRaftCandidate) -> tuple[float, ...]:
        preference_rank = 0 if candidate.berth_stern == prefer_stern else 1
        significant_seaward_bulge = candidate.seaward_bulge_px > _seaward_bulge_tolerance(candidate)

        if significant_seaward_bulge:
            natural_rank = 0 if candidate.berth_stern else 1
            if candidate.berth_stern:
                shape_penalty = candidate.seaward_bulge_px + (candidate.landward_recess_px * 0.35)
            else:
                shape_penalty = (candidate.seaward_bulge_px * 1.75) + (candidate.landward_recess_px * 0.6)
            return (
                1.0,
                float(natural_rank),
                shape_penalty,
                float(preference_rank),
                -candidate.straightness,
                float(-(candidate.capacity - required)),
            )

        neutral_penalty = candidate.seaward_bulge_px + (candidate.landward_recess_px * 0.6)
        orientation_tiebreak = 0.0 if not candidate.berth_stern else 1.0
        return (
            0.0,
            float(preference_rank),
            neutral_penalty,
...
def _place_coastal_alongside_raft_cluster(
    berth_water_mask: NDArray[np.bool_],
    occupancy: NDArray[np.bool_],
    berth_segments: list[_BerthSegment],
    svg_metas: list[_SvgMeta] | None,
    ...
    shipgen_kwargs: dict[str, Any] | None = None,
    berth_runs: list[_BerthRun] | None = None,
    min_cluster_ships: int = 2,
) -> list[str]:
    """Place a shoreline-attached alongside raft: one berthed lead, followers offshore."""
    labels: list[str] = []
    runs = list(berth_runs) if berth_runs is not None else _build_berth_runs(berth_segments, berth_water_mask)
    if not runs:
        return labels

    required_cluster_ships = max(2, min_cluster_ships)
    runs = [
        run
        for run in runs
        if _coastal_raft_run_capacity(
            run,
            resolution_m,
            length_range,
            berth_stern=False,
            required_cluster_ships=required_cluster_ships,
        ) >= required_cluster_ships
    ]
    if not runs:
        return labels
    ...
    for run in runs:
        ...
        anchor_sample = _sample_berth_run(run, run.length / 2.0)
        if anchor_sample is None:
            continue
        anchor_x, anchor_y, tan_x, tan_y, water_nx, water_ny = anchor_sample
        angle_deg = _angle_deg_from_stern_direction(tan_x, tan_y)
        angle_rad = math.radians(angle_deg)
        ...
        for index in range(n_ships):
            ...
            local_hull = _local_hull_geometry(svg_text_i, bw, lh, angle_deg)
            min_row, max_row = _geometry_projection_extents(local_hull, water_nx, water_ny)
            min_t, max_t = _geometry_projection_extents(local_hull, tan_x, tan_y)
            ship_specs.append(
                (
                    svg_text_i,
                    bw,
        ...
        active_ship_specs = list(ship_specs)
        while active_ship_specs:
            max_span = max(spec[7] for spec in active_ship_specs)
            if max_span <= run.length:
                break
            longest_index = max(range(len(active_ship_specs)), key=lambda idx: active_ship_specs[idx][7])
            active_ship_specs.pop(longest_index)

        if len(active_ship_specs) < required_cluster_ships:
            continue

        while active_ship_specs:
            lead_index = max(range(len(active_ship_specs)), key=lambda idx: active_ship_specs[idx][7])
            if lead_index != 0:
                active_ship_specs[0], active_ship_specs[lead_index] = active_ship_specs[lead_index], active_ship_specs[0]

            def _candidate_center(row_offset: float, tangent_offset: float = 0.0) -> tuple[float, float]:
                return (
                    anchor_x + water_nx * row_offset + tan_x * tangent_offset,
                    anchor_y + water_ny * row_offset + tan_y * tangent_offset,
                )
            ...
            def _contact_candidate(
                ...
                min_row: float,
                row_half: float,
                prev_row_offset: float,
                prev_max_row: float,
                prev_row_half: float,
                prev_hull: BaseGeometry,
            ) -> tuple[float, float, float, BaseGeometry] | None:
                obb_penetration_limit = max(1.0, min(prev_row_half, row_half) * 0.22)
                ...
                def _sample_contact(row_offset: float) -> tuple[float, float, float, BaseGeometry] | None:
                    penetration = prev_row_offset + prev_max_row - (row_offset + min_row)
                    obb_penetration = prev_row_offset + prev_row_half - (row_offset - row_half)
                    if penetration > 1.0 or obb_penetration > obb_penetration_limit:
                        return None

                    cx, cy = _candidate_center(row_offset)
                    if (
                        local_min_x + cx < 0.0
            ...
            base_placements: list[
                tuple[str, int, int, tuple[int, int, int, int], BaseGeometry, float, float]
            ] = []
            prev_row_offset = 0.0
            prev_max_row = 0.0
            prev_row_half = 0.0
            ...
            for index, (
                ...
                _tangent_span,
                local_hull,
            ) in enumerate(active_ship_specs):
                if index == 0:
                    row_offset = -min_row
                    base_cx, base_cy = _candidate_center(row_offset)
                    hull_geom = _translate_hull_geometry(local_hull, base_cx, base_cy)
                else:
                    if prev_hull is None:
                        valid = False
                        failed_index = index
                        break
                    base_contact_row = prev_row_offset + prev_max_row - min_row
                    ...
                    if contact is None:
                        ...
                        failed_index = index
                        break
                    row_offset, base_cx, base_cy, hull_geom = contact
                ...
                resolved = _resolve_berth_land_intrusion(
                    ...
                    water_nx,
                    water_ny,
                    max_shift_px=max(6.0, row_offset + 3.0),
                )
                if resolved is None:
                    valid = False
                    failed_index = index
                ...
                resolved_cx, resolved_cy, _resolved_geom = resolved
                required_shift = max(
                    0.0,
                    ((resolved_cx - base_cx) * water_nx) + ((resolved_cy - base_cy) * water_ny),
                )
                required_shifts.append(required_shift)
                base_placements.append((svg_text_i, bw, lh, hull_fill, local_hull, base_cx, base_cy))
                prev_row_offset = row_offset
                prev_max_row = max_row
                prev_row_half = row_half
                prev_hull = hull_geom
            ...
            shared_shift = max(required_shifts, default=0.0)
            if shared_shift > 6.0 or shared_shift - min(required_shifts, default=shared_shift) > 4.0:
                active_ship_specs.pop()
                if len(active_ship_specs) < required_cluster_ships:
                    break
                continue
            ...
            failed_index = None
            for index, (svg_text_i, bw, lh, hull_fill, local_hull, base_cx, base_cy) in enumerate(base_placements):
                cx = base_cx + water_nx * shared_shift
                cy = base_cy + water_ny * shared_shift
                hull_geom = _translate_hull_geometry(local_hull, cx, cy)
                min_x, min_y, max_x, max_y = hull_geom.bounds
                if min_x < 0.0 or min_y < 0.0 or max_x > image_size or max_y > image_size:
                    failed_index = index
                ...
                if _geometry_intrudes_land(berth_water_mask, hull_geom):
                    failed_index = index
                    placed = []
                    break
                ...
                if placed and _signed_geometry_gap(placed[-1].hull_geom, hull_geom) > 0.8:
                    failed_index = index
                    placed = []
                    break
            ...
            staged_occupancy = occupancy.copy()
            for ship in placed:
                _stamp_geometry_occupancy(staged_occupancy, ship.hull_geom)
            occupancy[:] = staged_occupancy
            water_tint = _sample_water_tint(background, round(anchor_x), round(anchor_y))
            rendered_cluster = _render_vector_raft_cluster(
                placed,
                image_size,
                blur_sigma,
...
def _place_coastal_raft_cluster(
    berth_water_mask: NDArray[np.bool_],
    occupancy: NDArray[np.bool_],
    berth_segments: list[_BerthSegment],
    svg_metas: list[_SvgMeta] | None,
    ...
    """Place a shoreline-attached tight raft while preserving contact density."""
    if not berth_stern:
        return _place_coastal_alongside_raft_cluster(
            berth_water_mask,
            occupancy,
            berth_segments,
            svg_metas,
            ...
            min_cluster_ships=min_cluster_ships,
        )
    ...
    for run in runs:
        ...
        if target_ship_count < required_cluster_ships:
            continue

        anchor_s = run.length / 2.0
        anchor_sample = _sample_berth_run(run, anchor_s)
        if anchor_sample is None:
            continue
        anchor_x, anchor_y, tan_x, tan_y, water_nx, water_ny = anchor_sample
        stern_dx = -water_nx if berth_stern else tan_x
        ...
        for index in range(target_ship_count):
            ...
            ship_specs.append(
                (
                    ...
                    max_t,
                    -min_n,
                    _local_hull_geometry(svg_text_i, bw, lh, angle_deg),
                )
            )
        ...
        while active_ship_specs:
            total_span = sum(
                max_t - min_t
                for _svg_text_i, _bw, _lh, _hull_fill, min_t, max_t, _water_offset, _local_hull in active_ship_specs
            )
            if total_span <= run.length:
                break
            active_ship_specs.pop()
        ...
        for svg_text_i, bw, lh, hull_fill, min_t, max_t, water_offset, local_hull in active_ship_specs:
            center_offset = cursor - min_t
            line_x = anchor_x + tan_x * center_offset
            line_y = anchor_y + tan_y * center_offset
            cx = line_x + water_nx * water_offset
            cy = line_y + water_ny * water_offset
            hull_geom = _translate_hull_geometry(local_hull, cx, cy)
            min_x, min_y, max_x, max_y = hull_geom.bounds
            if min_x < 0.0 or min_y < 0.0 or max_x > image_size or max_y > image_size:
                valid = False
            ...
            resolved = _resolve_berth_land_intrusion(
                ...
                water_nx,
                water_ny,
                max_shift_px=max(6.0, water_offset + 3.0),
            )
            if resolved is None:
                valid = False
                break
            ...
            required_shift = max(
                0.0,
                ((resolved_cx - cx) * water_nx) + ((resolved_cy - cy) * water_ny),
            )
            required_shifts.append(required_shift)
            base_placements.append((svg_text_i, bw, lh, hull_fill, local_hull, cx, cy))
            cursor = center_offset + max_t
        ...
        if shared_shift > 4.0:
            continue
        if shared_shift - min(required_shifts, default=shared_shift) > 3.0:
            continue

        pre_occupancy = occupancy.copy()
        placed: list[_RaftShipPlacement] = []
        for svg_text_i, bw, lh, hull_fill, local_hull, base_cx, base_cy in base_placements:
            cx = base_cx + water_nx * shared_shift
            cy = base_cy + water_ny * shared_shift
            hull_geom = _translate_hull_geometry(local_hull, cx, cy)
            min_x, min_y, max_x, max_y = hull_geom.bounds
...
def _place_cluster(
    ...
    if (
        ...
        runs = list(berth_runs) if berth_runs is not None else _build_berth_runs(berth_segments, berth_water_mask)
        if runs:
            coastal_required_ships = max(2, coastal_raft_min_ships)
            coastal_candidates = _ordered_coastal_raft_candidates(
                runs,
                resolution_m,
                length_range,
                rng,
                berth_stern_prob=berth_stern_prob,
                min_required_ships=coastal_required_ships,
            )
            if (
                n_ships >= coastal_required_ships
                and coastal_candidates
                and rng.random() < max(0.0, min(1.0, coastal_raft_tight_prob))
            ):
                coastal_rng = _fork_rng(rng)
                for candidate in coastal_candidates:
                    coastal_labels = _place_coastal_raft_cluster(
                        berth_water_mask,
                        occupancy,
                        berth_segments,
                        svg_metas,
                        ...
                        size_thresholds,
                        mixed,
                        berth_stern=candidate.berth_stern,
                        blur_sigma=blur_sigma,
                        shadow_azimuth_rad=shadow_azimuth_rad,
                        shadow_length=shadow_length,
                        shadow_alpha=shadow_alpha,
                        ...
                        sensor_az_world_deg=sensor_az_world_deg,
                        shipgen_kwargs=shipgen_kwargs,
                        berth_runs=[candidate.run],
                        min_cluster_ships=coastal_required_ships,
                    )
                    if coastal_labels:
                        rng.setstate(coastal_rng.getstate())
                        return coastal_labels

            berthed_modes = _ordered_berth_mode_attempts(
                runs,
                resolution_m,
                length_range,
                rng,
            ...
            for berth_stern in berthed_modes:
