...
def _write_dataset_yaml(
    ...
    *,
    ...
    resolved_path = path_value if path_value is not None else dataset_root.as_posix()
...
class TestProcessLabelFile:
    ...
    def test_avoid_overlap_allows_overlap_between_expanded_boxes(self, dataset) -> None:
        """衝突判定は他の拡張前OBBに対してのみ行う。"""
        lbl_path = dataset / "labels" / "train" / "test.txt"
        lbl_path.write_text(
            "0 0.200000 0.300000 0.400000 0.300000 "
            "0.400000 0.700000 0.200000 0.700000\n"
            "1 0.600000 0.300000 0.800000 0.300000 "
            "0.800000 0.700000 0.600000 0.700000\n"
        )
        ...
        assert result["updated"] == 1
        assert result["labels_expanded"] == 2
        ...
        assert first_width == pytest.approx(50.0, abs=1e-4)
        assert second_width == pytest.approx(50.0, abs=1e-4)
        assert _overlap_area(first, second) > 0.0

    def test_avoid_overlap_scales_back_against_original_obstacles(self, dataset) -> None:
        """拡張後OBBは他の拡張前OBBに当たる直前まで縮退する。"""
        lbl_path = dataset / "labels" / "train" / "test.txt"
        original_text = (
            "0 0.200000 0.300000 0.400000 0.300000 "
            "0.400000 0.700000 0.200000 0.700000\n"
            "1 0.450000 0.300000 0.650000 0.300000 "
            "0.650000 0.700000 0.450000 0.700000\n"
        )
        lbl_path.write_text(original_text)

        original_lines = original_text.strip().splitlines()
        original_first = _label_corners(original_lines[0])
        original_second = _label_corners(original_lines[1])
        ...
        assert first_width == pytest.approx(30.0, abs=2e-3)
        assert second_width == pytest.approx(30.0, abs=2e-3)
        assert _overlap_area(first, original_second) == pytest.approx(0.0, abs=1e-8)
        assert _overlap_area(second, original_first) == pytest.approx(0.0, abs=1e-8)
        assert _overlap_area(first, second) > 0.0
...
class TestCliExpandObb:
    def test_cli_calls_expand_obb_dataset_with_config(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """CLI の expand-obb は --config を使って処理関数を呼ぶ。"""
        ...
        with patch("medetect.yolo.__main__.expand_obb_dataset") as mock_fn:
            main()
        ...
        mock_fn.assert_called_once_with(
            ...
            expand_height=0.0,
            expand_width=4.0,
            expand_height_weighted=0.0,
            expand_width_weighted=0.0,
            avoid_overlap=True,
            max_workers=None,
        )
