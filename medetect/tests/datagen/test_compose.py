...
class TestComposeBerth:
    ...
    def test_debug_require_berth_retries_until_segments_exist(
        self,
        water_tif: pathlib.Path,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """debug_require_berth は berth segment が出る crop まで選び直す。"""
        from shapely.geometry import LineString
        ...
        def _capture_segments(*args, **kwargs):
            del args, kwargs
            segment_calls.append(len(segment_calls))
            if len(segment_calls) == 1:
                return []
            return [((8.0, 12.0), (56.0, 12.0))]

        def _capture_berth_helper(*args, **kwargs):
            berth_calls.append({"n_ships": args[6], "berth_stern": kwargs["berth_stern"]})
            return ["berthed-label"]

        monkeypatch.setattr(compose_mod, "_coastline_to_pixel_segments", _capture_segments)
        monkeypatch.setattr(compose_mod, "_place_berthed_cluster", _capture_berth_helper)
        ...
        result = _compose_one(
            ...
            rng=random.Random(3),
            max_crop_attempts=2,
            debug_require_berth=True,
        )

        assert result is not None
        _tile, labels, _n_clusters = result
        assert labels == ["berthed-label"]
